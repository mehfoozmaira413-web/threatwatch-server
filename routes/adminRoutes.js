...
const { ObjectId } = require("mongodb"); // <-- YE ADD KIYA
    ...
    try {
      ...
      if (!allowedRoles.includes(role)) {
        return res.status(400).json({
          message:
            "Invalid role. Role must be User, Moderator, or Admin.",
        });
      }
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      // 1. Pehle user ke saare scans delete karo
      await Scan.deleteMany({ user: req.params.id });

      // 2. Phir user ko delete karo
      ...
      return res.status(200).json({
        success: true,
        message: "User and all scans deleted successfully.",
      });
    } catch (error) {
      console.error(
        "DELETE USER ERROR:",
